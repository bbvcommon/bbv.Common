//-------------------------------------------------------------------------------
// <copyright file="DistributedEventBrokerExtensionBaseTest.cs" company="bbv Software Services AG">
...
namespace bbv.Common.DistributedEventBroker
{
    using System;
    ...
    public class DistributedEventBrokerExtensionBaseTest : IDisposable
    {
        ...
        public void Manage_MustRegisterDynamicTopic()
        {
            ...
            this.internalRegisterer.Verify(registerer => 
                registerer.AddSubscription("topic://bbv.Common.DistributedEventBroker/TestDistributedEventBroker", this.testee, It.IsAny<EventHandler<EventArgs<IEventFired>>>(), It.IsAny<IHandler>()));
        }
