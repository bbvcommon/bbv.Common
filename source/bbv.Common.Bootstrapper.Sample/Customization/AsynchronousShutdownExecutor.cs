//-------------------------------------------------------------------------------
// <copyright file="AsynchronousShutdownExecutor.cs" company="bbv Software Services AG">
...
namespace bbv.Common.Bootstrapper.Sample.Customization
{
    ...
    /// </summary>
    public class AsynchronousShutdownExecutor : IExecutor<IComplexExtension>
    {
        /// <inheritdoc />
        public string Name
        {
        ...
        {
            return "Runs all executables asynchronously on the extensions in the reverse order which they were added.";
        }
            ...
            foreach (IExecutable<IComplexExtension> executable in syntax)
            {
                using (var worker = new Task(
                    state =>
                    {
                        ...
                        e.Execute(reversedExtensions, executableContext);
                    },
                    executable))
                {
                    worker.Start();
