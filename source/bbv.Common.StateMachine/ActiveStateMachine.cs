//-------------------------------------------------------------------------------
// <copyright file="ActiveStateMachine.cs" company="bbv Software Services AG">
...
namespace bbv.Common.StateMachine
{
    ...
    /// This state machine reacts to events on its own worker thread and the <see cref="Fire"/> or
    /// <see cref="FirePriority"/> methods return immediately back to the caller.
    /// </summary>
        ...
        where TEvent : IComparable
    {
        ...
        {
            this.moduleController.EnqueueMessage(new EventInformation<TEvent>(eventId, eventArguments));
        ...
        {
            this.moduleController.EnqueuePriorityMessage(new EventInformation<TEvent>(eventId, eventArguments));
        ...
        public void Execute(EventInformation<TEvent> message)
        {
            ...
            this.stateMachine.Fire(message.EventId, message.EventArguments);
        }
