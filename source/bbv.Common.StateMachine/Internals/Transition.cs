//-------------------------------------------------------------------------------
// <copyright file="Transition.cs" company="bbv Software Services AG">
...
namespace bbv.Common.StateMachine.Internals
{
        ...
        where TEvent : IComparable
    {
        ...
        public ITransitionResult<TState, TEvent> Fire(ITransitionContext<TState, TEvent> context)
        {
            ...
            if (!this.ShouldFire(context.EventArguments, context))
            {
            ...
            if (!this.InternalTransition)
            {
                ...
                this.Fire(this.Source, this.Target, context.EventArguments, context);

                newState = this.Target.EnterByHistory(context);
            }
            else
            {
                this.PerformActions(context.EventArguments, context);
            }
        ...
        /// <param name="context">The event context.</param>
        private void Fire(IState<TState, TEvent> source, IState<TState, TEvent> target, object[] eventArguments, ITransitionContext<TState, TEvent> context)
        {
            ...
            else
            {
                ...
                {
                    source.Exit(context);
                    this.Fire(source.SuperState, target, eventArguments, context);
                }
                else if (source.Level < target.Level)
                {
                    // Handles 2.
                    // Handles 5c.
                    this.Fire(source, target.SuperState, eventArguments, context);
                    target.Entry(context);
                }
                else
                {
                    // Handles 5a.
                    source.Exit(context);
                    this.Fire(source.SuperState, target.SuperState, eventArguments, context);
                    target.Entry(context);
                }
            }
        }
        ...
        private bool ShouldFire(object[] eventArguments, ITransitionContext<TState, TEvent> context)
        {
            try
            {
        ...
        /// <param name="eventArguments">The event arguments.</param>
        /// <param name="context">The event context.</param>
        private void PerformActions(object[] eventArguments, ITransitionContext<TState, TEvent> context)
        {
