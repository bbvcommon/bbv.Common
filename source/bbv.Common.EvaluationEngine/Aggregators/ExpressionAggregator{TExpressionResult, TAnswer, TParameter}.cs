//-------------------------------------------------------------------------------
// <copyright file="ExpressionAggregator{TExpressionResult, TAnswer, TParameter}.cs" company="bbv Software Services AG">
...
namespace bbv.Common.EvaluationEngine.Aggregators
{
    ...
    public class ExpressionAggregator<TExpressionResult, TAnswer, TParameter> : IAggregator<TExpressionResult, TAnswer, TParameter>
    {
        ...
        {
            return string.Format("expression aggregator with seed '{0}' and aggregate function {1}", this.seed, this.aggregateFunc);
        }
    }
}