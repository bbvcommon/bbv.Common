//-------------------------------------------------------------------------------
// <copyright file="Engine.cs" company="bbv Software Services AG">
...
namespace bbv.Common.EvaluationEngine.Internals
{
    ...
    public class Engine : IEngine, IDefinitionHostProvider
    {
        ...
        public TAnswer Answer<TAnswer, TParameter>(IQuestion<TAnswer, TParameter> question, TParameter parameter)
        {
            ...
            CheckValueNotNull(definition, string.Format("No definition found. Set definition with Solve method first: question = {0}", question));

            var strategy = definition.GetStrategy<TAnswer, TParameter>();

            context.Strategy = strategy;

            CheckValueNotNull(strategy, string.Format("No strategy found. Set strategy with Solve method first: question = {0}", question));
