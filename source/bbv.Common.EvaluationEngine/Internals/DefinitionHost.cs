//-------------------------------------------------------------------------------
// <copyright file="DefinitionHost.cs" company="bbv Software Services AG">
...
namespace bbv.Common.EvaluationEngine.Internals
{
    ...
    public class DefinitionHost : IDefinitionHost
    {
        ...
        {
            var def = this.definitions.Where(d => d.QuestionType.IsAssignableFrom(question.GetType())).SingleOrDefault();
        ...
        {
            return this.definitions.Where(d => d.QuestionType.IsAssignableFrom(questionType)).SingleOrDefault();
        }
        ...
        private void CheckThatNoDefinitionWithSameQuestionTypeAlreadyExists(IDefinition definition)
        {
            ...
            {
                throw new InvalidOperationException(string.Format("A definition for question {0} was already added.", definition.QuestionType));
            }
        }
    }
}